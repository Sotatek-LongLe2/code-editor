...
import {
  ...
  CompressionMethods,
  ExternalPermissions,
  GeneratorPolynomialCRC32,
} from 'types/constants';

import { TFileTree, TZipEntry } from 'types/types';

import { concatUint8Arrays, createAndDownloadFile, getCurrentTimestamps, numberToBytes } from 'utils';

export class ZipWriter {
  private entries: TZipEntry[];
  private name: string;
  ...
  generate(): Uint8Array {
    ...
    for (const entry of this.entries) {
      ...
      zipContent = this.concatUint8Arrays(
        ...
        localFileHeader,
        compressedData,
        numberToBytes(Signatures.SPLIT_ZIP_FILE, 4),
      ...
      previousEntriesLength += localFileHeader.length + compressedData.length + DescriptorBytesLength;
    }
   ...
   * @returns {TZipEntry[]} An array of ZipEntry objects representing the file tree.
   */
  private convertFileTreeToZipEntries(fileTree: TFileTree, basePath = ''): TZipEntry[] {
  ...
  private createLocalFileHeader(
    ...
    const signature = Signatures.LOCAL_FILE_HEADER;
    const versionNeededToExtract = 0x0014; // Version 2.0
    const generalPurposeBitFlag = compressionMethod === CompressionMethods.DEFLATE ? 0x0008 : 0x0000;
  ...
  private createCentralDirectoryEntry(
    ...
    const signature = Signatures.CENTRAL_FILE_HEADER;
    ...
    const externalFileAttributes = entry.name.endsWith(DirectorySignature)
      ? ExternalPermissions.FOLDER
      : ExternalPermissions.FILE; // rwx
  ...
  private createEndOfCentralDirectory(
    ...
    const signature = Signatures.END_OF_CENTRAL_DIR;
    const diskNumber = 0;
  ...
  private calculateCRC32(data: Uint8Array): number {
    ...
    for (let i = 0; i < 256; i++) {
      let crc = i;
      for (let j = 0; j < 8; j++) {
        crc = crc & 1 ? (crc >>> 1) ^ GeneratorPolynomialCRC32 : crc >>> 1;
      }
      crcTable[i] = crc >>> 0;
    }
  ...
  private compressData(rawData: Uint8Array) {
    ...
    return compressedData.length < rawData.length
      ? {
          data: compressedData,
          method: CompressionMethods.DEFLATE,
        }
      : {
          data: rawData,
          method: CompressionMethods.STORE,
        };
  }
}
